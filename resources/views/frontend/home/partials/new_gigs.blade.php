<div class="new-services-section">
    <div class="container">
        ...
        <div class="listing-tab" data-aos="fade-up">
            <div class="listing-slider">
                <ul class="nav nav-tabs justify-content-center">
                    ...
                    <li class="nav-item">
                        <a href="javascript:void(0);" class="nav-link @if($k == 0) active @endif" data-bs-toggle="tab"
        ...
        <div class="tab-content" data-aos="fade-up">
            @if(!empty($categories) && count($categories) > 0)
            @foreach($categories as $k => $category)
            <div class="tab-pane fade @if($k == 0) show active @endif" id="category_{{ $category->id }}"
                role="tabpanel">
                @php
                $services = collect($section['section_content'])->where('category_id', $category->id)->take(6);
                @endphp
                <div class="row">
                    ...
                    <div class="col-xl-4 col-md-6">
                        <div class="gigs-grid">
                            <div class="gigs-img">
                                <div class="img-slider owl-carousel" role="region"
                                    aria-label="Image carousel for {{ $service['title'] ?? 'gig' }}">
                                    @if(!empty($service['gig_image']) && count($service['gig_image']) > 0)
                                    @foreach($service['gig_image'] as $image)
                                    <div class="slide-images">
                                        <a href="{{ route('service.detail', $service['slug']) }}">
                                            <img src="{{ $image }}" class="img-fluid"
                                                alt="{{ $service['title'] ?? 'gig' }}">
                                        </a>
                                    </div>
                                    @endforeach
                                    @endif
                                ...
                                <div class="card-overlay-badge">
                                    ...
                                    <a href="{{ route('service.detail', $service['slug']) }}"><span
                                ...
                                <div class="gigs-card-footer">
                                    <div class="d-flex align-items-center gigs-left-text">
                                        <a href="#" class="avatar avatar-sm flex-shrink-0"><img
                                                ...
                                                alt="Provider Profile"></a>
                                        <div class="ms-2">
                                            <h6 class="mb-0"><a href="#">{{ $service['provider_name'] }}</a></h6>
                    ...
                    <div class="col-md-12 text-center">
                        <p>{{__('web.home.no_data_found_for')}} {{ $category->name }}</p>
                    </div>
                    @endforelse
                </div>
            </div>
