        ...
        <div class="content me-4">
            ...
            <div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-3">
                <div class="my-auto mb-2">
                    <h4 class="mb-1">{{ __('admin.finance.buyer_earning') }}</h4>
                    <nav>
                        <ol class="breadcrumb mb-0">
                            <li class="breadcrumb-item">
                                <a href="{{ route('dashboard') }}">{{ __('admin.common.home') }}</a>
                            </li>
                            <li class="breadcrumb-item active" aria-current="page">{{ __('admin.finance.buyer_earning') }}
                            </li>
                        </ol>
                    </nav>
                </div>
                ...
                <table class="table" id="earningTable">
                    <thead class="thead-light">
                        <tr>
                            ...
                            <th>{{ strtoupper(__('admin.finance.booking')) }}</th>
                            <th>{{ strtoupper(__('admin.finance.total_earning')) }}</th>
                            <th>{{ strtoupper(__('admin.finance.admin_earning')) }}</th>
                            <th>{{ strtoupper(__('admin.finance.buyer_pay_due')) }}</th>
                            <th>{{ strtoupper(__('admin.finance.buyer_requested_amount')) }}</th>
                            <th>{{ strtoupper(__('admin.finance.buyer_remaining_amount')) }}</th>
                            @if (hasPermission($permissions, 'buyer_earning', 'edit') || hasPermission($permissions, 'buyer_earning', 'delete'))
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="mb-0">{{ __('admin.finance.buyer_earning') }}</h5>
                    <button type="button" class="btn-close custom-btn-close" data-bs-dismiss="modal"
                        aria-label="Close">
                        <i class="ti ti-x fs-16"></i>
                    </button>
                </div>
                <form id="adminPayoutForm" autocomplete="off">
                        ...
                        <div class="row">
                            ...
                            <div class="mb-3">
                                <div id="codUploadSection" class="">
                                    <label for="provider_amount"
                                        class="form-label">{{ __('admin.common.amount') }}</label>
                                    <input type="number" class="form-control mb-2" id="provider_amount"
                                        name="provider_amount" step="0.01" min="0">
                                    <span id="provider_amount_error" class="text-danger"
                                        style="display: none;">{{ __('amount_exceed') }}</span>

                                    <label for="codFile"
                                        class="form-label mt-2">{{ __('admin.common.upload_proof') }}</label>
...
@push('scripts')
    <script src="{{ asset('backend/assets/js/finance/earning.js') }}"></script>
@endpush
