...
function renderBuyerCard(seller) {
    return `
...
function fetchMyBuyers(page = 1) {
    $.ajax({
        type: "GET",
        url: "/seller/my-buyers-list?page=" + page,
        beforeSend: function () {
        ...
        success: function (res) {
            if (res.data.length) {
                $.each(res.data, function (i, seller) {
                    $("#my_buyers_list").append(renderBuyerCard(seller));
                });
                ...
                $("#my_buyers_list").html(`<p class="text-center">${_l("web.user.no_buyers_found")}</p>`);
                $(".load_more_btn").addClass("d-none");
            }
        },
        error: function (xhr) {
