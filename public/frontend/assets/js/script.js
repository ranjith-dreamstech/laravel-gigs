...
(function ($) {
	...
	if($('.img-slider').length > 0) {
		$('.img-slider').owlCarousel({
			...
			autoplay:false,
			navText: [
				'<i class="fa-solid fa-chevron-left" aria-hidden="true"></i><span class="sr-only">Previous slide</span>',
				'<i class="fa-solid fa-chevron-right" aria-hidden="true"></i><span class="sr-only">Next slide</span>'
			],
			responsive:{
				0:{
					items:1
		...
		$('.img-slider .owl-item').not('.cloned').each(function (index) {
		$(this)
			.attr('role', 'group')
			.attr('aria-label', 'Slide ' + (index + 1) + ' of ' + $('.img-slider .owl-item').not('.cloned').length);
		});
   ...
   if ($('.select').length > 0) {
	...
	if($('.viewall-one').length > 0) {
		...
		$(".viewall-button-one").on("click", function() {
	 		$(this).text($(this).text() === "Less Categories" ? "More Categories" : "Less Categories");
