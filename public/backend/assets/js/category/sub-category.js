...
/* global $, loadTranslationFile, document, showToast, _l, FormData, FileReader, Image, URL, loadUserPermissions, hasPermission*/
    ...
    $("#subCategoryForm").validate({
        ...
        messages: {
            ...
            description: {
                ...
                maxlength: _l('admin.common.description_maxlength', { max: 255 }),
            },
            category_id: {
                required: _l('admin.common.category_required'),
            }
        },
            ...
            $.ajax({
                ...
                success: function (resp) {
                    ...
                    if (resp.code === 200) {
                        ...
                        $("#subCategoryTable").DataTable().ajax.reload(); // adjust table ID if needed
                    }
...
$('#image').on('change', function (event) {
    ...
    if (file) {
        ...
        img.onload = function () {
            URL.revokeObjectURL(objectURL); // still clean up the objectURL
        };
        img.src = objectURL;
    }
});
...
$('.sort_by_list .dropdown-item').on('click', function () {
    ...
    $('#sortBy').text(sortText);
    $('#sort_by_input').val(sortType);

    if (['ascending', 'descending'].includes(sortType)) {
        $('#sort_by_status').val(sortType);
    } else if (['last month', 'last 7 days'].includes(sortType)) {
        $('#sort_by_status').val(sortType);
    } else {
        $('#sort_by_status').val('');
    }
    ...
    $("#subCategoryTable").DataTable({
        ...
        ajax: {
            ...
            data: function (d) {
                ...
                d.sort_status = $('#sort_by_status').val();
            },
            headers: {
                "Accept": "application/json",
                "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
            ...
            complete: function () {
                ...
                if ($("#subCategoryTable").DataTable().rows().count() === 0) {
        ...
        columns: [
            ...
            { data: "parent_name" },
            {
                ...
                render: function (data, type, row) {
                    return data === 'active'
                        ? `<span class="badge bg-primary">${_l("admin.common.active")}</span>`
                        : `<span class="badge bg-danger">${_l("admin.common.inactive")}</span>`;
                }
            },
            {
                data: "featured",
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    return `
                        ${(hasPermission(permissions, 'sub_category', 'edit') || hasPermission(permissions, 'sub_category', 'delete')) ? `
                            <div class="dropdown">
                                <button class="btn btn-icon btn-sm" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                    <i class="ti ti-dots-vertical"></i>
                                </button>
                                <ul class="dropdown-menu dropdown-menu-end p-2">
                                    ${hasPermission(permissions, 'sub_category', 'edit') ? `
                                        <li>
                                    ...
                                    ${hasPermission(permissions, 'sub_category', 'delete') ? `
                                        <li>
                                            <a class="dropdown-item rounded-1" href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#delete_modal" onclick="deleteCategory(${row.id});">
...
function editCategory(id, categoryname, slug, description, image, icon, feature, status, parentId) {
    ...
    $('#editCategoryModal #category_id').val(parentId).trigger('change');

    if (image) {
        let imagePath = '/storage/' + image.replace(/^\/+|\/+$/g, '');
        $('#editCategoryModal #editImagePreview').removeClass('d-none').attr('src', imagePath);
    } else {
        $('#editCategoryModal #editImagePreview').addClass('d-none').attr('src', '');
    }

    if (icon) {
        let iconPath = '/storage/' + icon.replace(/^\/+|\/+$/g, '');
        $('#editCategoryModal #editIconPreview').removeClass('d-none').attr('src', iconPath);
    } else {
        $('#editCategoryModal #editIconPreview').addClass('d-none').attr('src', '');
    }
...
function previewImage(event, previewId) {
    ...
    if (file) {
        ...
        reader.onload = function (e) {
            ...
            frames.find(".upload_icon").addClass("d-none");
        };
